...
- **Packaging:** `cargo pgrx package` per major (Linux/macOS/Windows, including a Windows build, mirroring the fork's release pipeline). `CREATE EXTENSION pgl_validate;` on **every** participant; scheduling additionally needs `shared_preload_libraries = 'pgl_validate'`. No user-schema changes; all objects under `pgl_validate`. No install-time dependency on pglogical (its catalogs are read opportunistically). Standard `--X.Y.sql` / `--X.Y--A.B.sql` scripts.
...
CREATE TABLE pgl_validate.fence_barrier (
    id          bigint GENERATED ALWAYS AS IDENTITY,  -- local surrogate; NOT replicated-identity-bearing
