# pgl_validate — Cross-Node Data Validation for PostgreSQL Replication
...
`pgl_validate` proves whether two or more PostgreSQL nodes actually hold the data they are *contractually supposed* to hold, using the two universally understood signals: **row counts** and **content checksums**. The primary target is pglogical replication, especially bidirectional topologies; native logical replication and physical standbys are supported backend modes with their own contracts. Replication can report healthy while data silently diverges (a `keep_local` conflict, a manual subscriber write, an interrupted initial COPY, a skipped apply transaction, decoding edge cases). No first-class, trustworthy verifier exists today.
...
- `conflict_evidence` — optional pglogical conflict-history rows correlated to confirmed divergences by subscription, relation, time window, and tuple JSON containing the divergent key. This is explanatory evidence (`update_update` + `keep_local`, `skip`, etc.), never a source of validation truth.
  Vanilla pglogical deployments may not expose `pglogical.conflict_history`; absence of that table is a normal, supported state that must return zero conflict evidence without weakening or failing validation. Queryable conflict history from the willibrandon pglogical fork enriches reports only. The fork records tuple JSON scalar values through PostgreSQL type output strings, so correlation must match both typed JSON keys and their stringified pglogical forms.
...
- **Version-sensitive surfaces** are isolated by layer: Rust PG-major ABI differences live in `src/compat/` and are gated with `#[cfg(feature = "pgNN")]`; SQL/catalog compatibility decisions live in the bootstrap SQL that owns them (`010_contracts.sql` for the per-type `send`-format stability table, `030_fencing.sql`/`050_compare.sql` for origin progress, snapshots, and generated `REPEATABLE READ` remote SQL).
...
- **Packaging:** `cargo pgrx package` per major with Linux/macOS `.tar.gz` artifacts and Windows `.zip` artifacts, plus a WiX v5 per-machine MSI for each Windows x64 PostgreSQL major. MSI installers must mirror the pglogical fork's release expectations: registry-based PostgreSQL path detection, explicit `POSTGRESQLDIR` override, browse fallback, clean uninstall, major-version-specific UpgradeCode values for side-by-side pg15/pg16/pg17/pg18 support, and CI verification with silent install/uninstall. macOS packages must rewrite `libpq.5.dylib` to `@rpath/libpq.5.dylib`, carry an `@loader_path` `LC_RPATH`, be ad-hoc re-signed, and fail packaging if any `.pgrx`/runner path remains; Linux packages must fail if dynamic loader metadata leaks build-runner paths. `pgl_validate.control` fixes `schema = pgl_validate` so PostgreSQL records the extension under its schema, matching pglogical's user-visible layout. `CREATE EXTENSION pgl_validate;` on **every** participant; scheduling additionally needs `shared_preload_libraries = 'pgl_validate'`. No user-schema changes; all objects under `pgl_validate`. pglogical support is a mandatory release and regression-test gate for the pglogical backend, and pglogical must be installed on pglogical participants. It is not a runtime prerequisite for native logical or physical standby validation. CI must cover both the Windows-capable fork release path and upstream vanilla pglogical source on platforms where upstream ships a normal source build; queryable conflict history from the fork is report enrichment only, never a validation precondition. Native logical and standby modes are additional backend paths, not a reason to weaken pglogical coverage. Standard `--X.Y.sql` / `--X.Y--A.B.sql` scripts.
- **Release tags:** the first public release tag was `v0.1.0`; each release tag must match `Cargo.toml` and `pgl_validate.control`. Cut releases only after the implementation goal is met, CI is green, release artifacts include Linux/macOS `.tar.gz` packages, Windows `.zip` packages, Windows x64 MSI installers, and checksums, and the documentation reflects the shipped behavior.
