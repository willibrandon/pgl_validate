...
- **Initial release:** the first public release tag is `v0.1.0`, matching `Cargo.toml` and `pgl_validate.control`. Cut it only after the implementation goal is met, CI is green, release artifacts include Linux/macOS `.tar.gz` packages, Windows `.zip` packages, Windows x64 MSI installers, and checksums, and the documentation reflects the shipped behavior. License is MIT, copyright Brandon Williams.
