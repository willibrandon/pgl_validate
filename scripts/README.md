...
- `install-pglogical-release.ps1` installs pglogical for pglogical-backed
  tests, the full pgrx test suite, and packaging jobs. It does not depend on a
  local pglogical clone.
...
- `package-pgrx.ps1` builds the release archive from `cargo pgrx package`:
  `.tar.gz` for Linux and macOS, `.zip` for Windows. It also verifies native
  loader metadata so release artifacts do not retain build-runner paths.
...
`check-public-comments.ps1` verifies that PostgreSQL-visible objects and
catalog columns have `COMMENT ON` coverage, and public Rust items have rustdoc.
